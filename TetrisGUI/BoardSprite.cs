            ...
            for (int i = 0; i < board.GetLength(0); i++)
                for (int j = 0; j < board.GetLength(1); j++)
                {
                    if (board[i, j] == Color.Transparent)
                        spriteBatch.Draw(emptyBlock, new Vector2(i* emptyBlock.Height,Math.Abs(19 -j) * emptyBlock.Width), Color.White);
                    else
                        spriteBatch.Draw(filledBlock, new Vector2(i * filledBlock.Height,Math.Abs(19 -j) * filledBlock.Width), board[i, j]);
                }
